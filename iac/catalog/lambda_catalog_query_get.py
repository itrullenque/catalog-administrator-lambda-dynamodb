...
class LambdaCatalogQueryGet:
    def __init__(
        ...
        self.stack = stack
        self.id = "lambda_catalog_query_get_lambda"
        self.name = "lambda_catalog_query_get"
    ...
    def __create_function(self):
        func = aws_lambda.Function(
            ...
            code=aws_lambda.Code.from_asset(path="iac/assets/lambda_catalog_query_get"),
            role=self.role(),
            memory_size=1024,
        )
        return func
    ...
    def role(self):
        ...
        lambda_dynamodb_policy_statement.add_actions(
            "dynamodb:getItem",
            "dynamodb:Query",
        )

        lambda_dynamodb_policy_statement.add_resources(
            f"arn:aws:dynamodb:*:*:table/{self.catalog_table_name}/index/academic_year-index",
            f"arn:aws:dynamodb:*:*:table/{self.catalog_table_name}/index/course_id-index",
        )
